...
const css=`#mrc-ov{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:2147483647;display:flex;align-items:center;justify-content:center}
...
function style(){if($('#mrc-style'))return;const s=document.createElement('style');s.id='mrc-style';s.textContent=css;document.head.appendChild(s)}
function mk(t,a={},h=''){const e=document.createElement(t);for(const k in a)e.setAttribute(k,a[k]);if(h)e.innerHTML=h;return e}
function openUI(){style();const ov=mk('div',{id:'mrc-ov',role:'dialog','aria-modal':'true'});
...
// ===== 自動スクロール + 「さらに/もっと」クリック =====
async function loadAllList(){
  // できるだけ下まで読み込む：高さが安定2回続くまで or 最大40サイクル
  let prev=0, stable=0;
  for(let i=0;i<40;i++){
    window.scrollTo(0,document.body.scrollHeight);
    // 「さらに読み込む」「もっと見る」系があれば叩く
...
// ===== 詳細URL収集（href / onclick / data-href ） =====
function collectDetailUrls(){
  ...
  $$('a[href*="/maimai-mobile/record/playlogDetail/"],a[href*="playlogDetail/"]').forEach(a=>{
    try{
      ...
      const url=raw.startsWith('http')?raw:new URL(raw,location.href).href;
      if(/playlogDetail/.test(url)) set.add(url);
    }catch{}
  });

  // 2) onclickに含まれているパターン
  $$('[onclick]').forEach(el=>{
    const s=String(el.getAttribute('onclick')||'');
    const m=s.match(/['"](\/?maimai-mobile\/record\/playlogDetail\/\?[^'"]+)['"]/)
          ||s.match(/['"](\/maimai-mobile\/record\/playlogDetail\/[^'"]+)['"]/)
          ||s.match(/['"]((?:\.?\/)?playlogDetail\/\?[^'"]+)['"]/);
    if(m){
      try{
        const raw=m[1].startsWith('/')?m[1]:'/maimai-mobile/record/'+m[1].replace(/^\.\//,'');
...
async function main(){
  ...
  // 事前確認
  setMsg('履歴データを取得・送信します。よろしければ「開始」。');
  ...
  async function start(){
    ...
    let urls=collectDetailUrls();
    if(!urls.length){
      setMsg('履歴の詳細リンクが見つかりませんでした。<br>一覧を下まで読み込んでから再試行してください。');
      setSub(`API: ${API_URL} / Bearer: ${TOKEN.slice(0,6)}…${TOKEN.slice(-4)}`);
      setBar(0); setBtns('閉じる','再試行',start,false);
      return;
    }

    setMsg('履歴データを取得・送信中…'); let ok=0,ng=0;
    for(let i=0;i<urls.length;i++){
      setBar((i/urls.length)*100); setSub(`進捗: ${i}/${urls.length}　成功:${ok}　失敗:${ng}`);
      try{
        const res=await fetch(urls[i],{credentials:'include'}); const html=await res.text();
        await postOne(urls[i],html); ok++;
      }catch(e){ ng++; }
    }
    setBar(100); setSub('');
